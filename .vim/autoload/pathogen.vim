...
function! pathogen#runtime_append_all_bundles(...) " {{{1
  ...
  for dir in pathogen#split(&rtp)
    if dir =~# '\<after$'
      ...
      " Added check for a vim subdirectory.  This allows sparkup to be loaded.
      let list +=  [dir] + filter(pathogen#glob_directories(dir.sep.name.sep.'*[^~]'), '!pathogen#is_disabled(v:val)') + filter(pathogen#glob_directories(dir.sep.name.sep.'*[^~]'.sep.'vim'), '!pathogen#is_disabled(v:val)')
    endif
  endfor
